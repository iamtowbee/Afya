...
const Navbar = () =>
{
  ...
  return (
    <header className='sticky top-0 z-50 w-full bg-white'>
      <nav className='container flex items-center shadow-md py-3 md:shadow-sm'>
        ...
        <div id='menu-btn' className="flex sm:hidden flex-1 justify-end pr-2">
          <ion-icon 
            name="menu" 
            className="p-3 rounded-md hover:cursor-pointer hover:bg-primary hover:text-white transition-all relative"
            onClick={() => setIsHamburgerOpen(!isHamburgerOpen)}></ion-icon>
        </div>
        {/* Hamburger Menu */}
        <div id={isHamburgerOpen ? "openHamburger" : "closedHamburger"} className="sm:hidden">
          <div id="ham-menu" className="absolute flex flex-col items-center self-end py-4 mt-9 text-xl font-semibold bg-white sm:w-auto sm:self-center left-12 right-12 drop-shadow-lg transition-all divide-y z-10">
            <a href="#" className='text-center w-full py-3 hover:bg-blue-50 text-primary hover:text-accent'>Home</a>
            <a href="#" className='text-center w-full py-3 hover:bg-blue-50 text-primary hover:text-accent'>Product</a>
